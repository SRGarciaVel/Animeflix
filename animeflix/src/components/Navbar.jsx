...
export function Navbar({ onSearchSelect, onRandomClick, syncSeason }) {
  const { malSession, repairADN, checkForNewEpisodes, importFromMAL, isRepairing } = useAnimeLibrary();
    ...
    <nav className="fixed top-0 w-full z-50 bg-black/80 backdrop-blur-lg border-b border-white/5 px-6 md:px-12 py-4 flex items-center justify-between transition-all">
      <div className="flex items-center gap-8">
        ...
        <div className="hidden md:flex items-center gap-6 ml-4 border-l border-white/10 pl-8">
          ...
          <Link to="/diary" className={`text-[11px] font-black uppercase tracking-widest transition-colors ${location.pathname === "/diary" ? "text-white" : "text-gray-500 hover:text-white"}`}>Diario</Link>
          ...
          <Link to="/news" className={`text-[11px] font-black uppercase tracking-widest transition-colors ${location.pathname === "/news" ? "text-white" : "text-gray-500 hover:text-white"}`}>Noticias</Link>
      ...
      <div className="flex items-center gap-4">
        ...
        <button onClick={checkForNewEpisodes} className="p-2 text-gray-400 hover:text-yellow-400 relative group"><BellRing size={20} /><span className="absolute top-1 right-1 h-2 w-2 bg-yellow-500 rounded-full animate-ping"></span></button>
        
        {/* BOTÓN REPARAR ADN EVIDENTE */}
        <button 
          onClick={() => repairADN()} 
          className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-netflix-red/10 border border-netflix-red/20 text-netflix-red hover:bg-netflix-red hover:text-white transition-all group"
        >
          <RefreshCw size={16} className={isRepairing ? "animate-spin" : "group-hover:rotate-180 transition-transform"} />
          <span className="text-[9px] font-black uppercase tracking-tighter">Reparar ADN</span>
        </button>
