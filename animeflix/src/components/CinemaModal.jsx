...
export function CinemaModal({ anime: initialAnime, onClose }) {
  const { myList, upsertAnime, deleteAnime, updateSmartStatus } = useAnimeLibrary();
  ...
  // --- ACTUALIZACIÓN INTELIGENTE DE EPISODIOS ---
  const handleUpdateProgress = async (newCount) => {
    ...
    // Calculamos el estado automáticamente usando la lógica de "Secuelas"
  ...
  const renderGenres = () => {
    ...
    return anime.genres.slice(0, 3).map((g, idx) => {
      ...
      const key = typeof g === 'object' ? (g.mal_id || idx) : `${g}-${idx}`;
      return (
        <span key={key} className="text-[8px] font-black uppercase border border-white/10 px-2 py-0.5 rounded-full opacity-40 italic">
          {genreName}
        </span>
      );
    });
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
      ...
      >
        ...
        <div className={`w-full md:w-[350px] p-10 flex flex-col items-center overflow-y-auto no-scrollbar border-r border-white/5 bg-black/20 transition-all duration-700 ${cinemaMode ? 'opacity-10 blur-xl scale-75 grayscale' : 'opacity-100'}`}>
          <img src={anime.image_url} className="w-56 rounded-3xl shadow-2xl mb-8 transform hover:scale-105 transition duration-1000 border-4" style={{ borderColor: accentColor }} alt="" />
        ...
        <div className="flex-1 p-12 md:p-16 overflow-y-auto no-scrollbar relative z-10">
          {cinemaMode && (
            <div className="mb-12 animate-in slide-in-from-bottom-10 duration-1000">
              ...
              <div className="relative aspect-video rounded-[3rem] overflow-hidden border border-white/10 bg-black" style={{ boxShadow: `0 0 100px ${accentColor}33` }}>
          ...
          <div className={`space-y-12 transition-all duration-700 ${cinemaMode ? 'opacity-40 blur-[4px] scale-95 origin-top' : 'opacity-100 scale-100'}`}>
            ...
            <div className="bg-black/40 p-10 rounded-[2.5rem] border border-white/5 shadow-2xl">
              <div className="flex justify-between items-end mb-10">
                <div>
                  <p className="text-[10px] font-black text-netflix-red uppercase italic mb-2 tracking-widest italic tracking-tighter">Tu Progreso Automático</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <button onClick={() => handleUpdateProgress(anime.episodes_watched + 1)} className="bg-white text-black py-5 rounded-[2.5rem] font-black text-xs hover:bg-gray-200 transition shadow-lg italic uppercase tracking-tighter">+1 Episodio</button>
                <button onClick={() => handleUpdateProgress(anime.episodes_watched + 12)} className="bg-gray-800 text-white py-5 rounded-[2.5rem] font-black text-xs hover:bg-gray-700 transition shadow-lg italic uppercase tracking-tighter">+1 Temporada</button>
              </div>
              <div className="relative mt-8 group">
                ...
                <input type="number" placeholder="Capítulo exacto..." className="w-full bg-white/5 border border-white/10 rounded-[2.5rem] py-6 text-center font-black text-2xl outline-none focus:border-netflix-red transition shadow-inner italic" onKeyDown={(e) => { 
                    if (e.key === 'Enter') { 
            ...
            <div className="bg-black/40 p-12 rounded-[3.5rem] border border-white/5 h-56 flex flex-col shadow-2xl">
              <p className="text-[11px] font-black text-gray-500 uppercase mb-6 flex items-center gap-4 tracking-[0.4em] italic tracking-tighter"><Edit3 size={20}/> Diario Personal</p>
            ...
            <button onClick={() => deleteAnime(anime.id)} className="w-full text-[11px] font-black text-red-500/10 hover:text-red-500 transition-all uppercase tracking-widest py-14 italic border border-transparent hover:border-red-500/20 rounded-[3rem] italic">Remover del Catálogo</button>
          </div>
        </div>
      </div>
    </div>
