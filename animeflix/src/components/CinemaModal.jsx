...
import { 
  ...
  GitMerge, LayoutGrid, ExternalLink, Trash2 
} from 'lucide-react';
...
export function CinemaModal({ anime: initialAnime, onClose }) {
  ...
  // Sincronización pro: usamos siempre la data más reciente del Hook
  const anime = myList.find(a => a.mal_id === initialAnime.mal_id) || initialAnime;
  const accentColor = useDynamicColor(anime.image_url);

  // --- FIX ESCRITURA: Estado local para notas para evitar lag ---
  ...
  const handleDelete = () => {
    if (window.confirm(`¿Estás seguro de que quieres eliminar "${anime.title}" de tu biblioteca?`)) {
      ...
      >
        ...
        <div className={`w-full md:w-[320px] p-8 flex flex-col items-center overflow-y-auto no-scrollbar border-r border-white/5 bg-black/20 transition-all duration-700 ${cinemaMode ? 'opacity-10 blur-xl scale-75 grayscale' : 'opacity-100'}`}>
          ...
          <div className="w-full space-y-4">
            ...
            <div className="bg-black/40 p-5 rounded-[2.5rem] border border-white/5 space-y-4 shadow-inner">
              <p className="text-[10px] font-black text-gray-500 uppercase text-center italic">Control Personal</p>
              ...
              <div className="space-y-2">
                <p className="text-[8px] font-black text-gray-600 uppercase tracking-widest text-center">Estado actual</p>
              ...
              <button 
                ...
                className="w-full mt-4 py-2 flex items-center justify-center gap-2 text-red-500/40 hover:text-red-500 transition-colors text-[9px] font-black uppercase tracking-widest"
              >
                <Trash2 size={12}/> Eliminar de biblioteca
              </button>
            </div>
          </div>
        </div>
          ...
          {cinemaMode && (
            <div className="mb-8 animate-in slide-in-from-bottom-10 duration-1000 text-center">
              <div className="flex gap-4 mb-4 justify-center">
                <button onClick={() => setVideoSource('trailer')} className={`px-5 py-1.5 rounded-full text-[9px] font-black uppercase transition-all ${videoSource === 'trailer' ? 'bg-white text-black' : 'bg-white/10 text-white'}`}><Youtube size={14}/> Trailer</button>
                <button onClick={() => setVideoSource('opening')} className={`px-5 py-1.5 rounded-full text-[9px] font-black uppercase transition-all ${videoSource === 'opening' ? 'text-white' : 'bg-white/10 text-white'}`} style={videoSource === 'opening' ? { backgroundColor: accentColor } : {}}><ListMusic size={14}/> Opening</button>
              </div>
              <div className="relative aspect-video rounded-[2.5rem] overflow-hidden border border-white/10 bg-black shadow-2xl" style={{ boxShadow: `0 0 100px ${accentColor}33` }}>
                {promoVideo && videoSource === 'trailer' ? (
                  <iframe 
                    className="w-full h-full" 
                    src={`https://www.youtube.com/embed/${promoVideo}?autoplay=1&mute=1&controls=1&rel=0&origin=${window.location.origin}&iv_load_policy=3&playsinline=1`} 
              ...
              <>
                <header>
                  <div className="flex items-center gap-3 mb-3">
                     <span className="text-[9px] font-black px-3 py-1 rounded-full uppercase shadow-lg" style={{ backgroundColor: accentColor }}>Original Animeflix</span>
                     <div className="flex gap-2">{renderGenres()}</div>
                     {fullMetadata && <AiringCountdown anime={anime} />}
                  </div>
                  <h2 className="text-5xl font-black italic uppercase leading-none mb-6 tracking-tighter drop-shadow-2xl">{anime.title}</h2>
                </header>
                ...
                <div className="bg-black/40 p-10 rounded-[2.5rem] border border-white/5 shadow-2xl">
                    ...
                    <div className="space-y-1">
                      ...
                      <h3 className="text-6xl font-black italic tracking-tighter leading-none flex items-baseline gap-2 italic">
                        ...
                        <AnimatePresence mode="wait">
                          <motion.span key={anime.episodes_watched} initial={{ y: 15, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -15, opacity: 0 }}>
                  ...
                  <div className="relative mt-10 group">
                    <Hash className="absolute left-10 top-1/2 -translate-y-1/2 text-gray-600 group-focus-within:text-netflix-red transition" size={24}/>
                    <input 
                      type="number" 
                      placeholder="Capítulo exacto..." 
                      className="w-full bg-white/5 border border-white/10 rounded-[2.5rem] py-6 text-center font-black text-2xl outline-none focus:border-netflix-red transition shadow-inner italic text-white" 
                      onKeyDown={(e) => { if (e.key === 'Enter') { handleUpdateProgress(parseInt(e.target.value) || 0); e.target.value = ''; } }} 
                    />
                  </div>
                </div>
                ...
                <div className="bg-black/40 p-12 rounded-[3.5rem] border border-white/5 h-64 flex flex-col shadow-2xl relative overflow-hidden">
                   <div className="flex justify-between items-center mb-6 relative z-10">
                     <p className="text-[11px] font-black text-gray-500 uppercase tracking-[0.4em] italic flex items-center gap-3"><Edit3 size={18} style={{ color: accentColor }}/> Diario</p>
                     <button onClick={() => isEditingNotes ? saveNotes() : setIsEditingNotes(true)} className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase transition-all border ${isEditingNotes ? 'bg-netflix-red text-white' : 'bg-white/5 text-gray-500'}`}>
                   ...
                   {isEditingNotes ? (
                     ...
                     <div className="prose prose-invert prose-sm italic opacity-70 overflow-y-auto no-scrollbar">
                        <ReactMarkdown>{anime.notes || "_¿Qué piensas de esta serie?_"}</ReactMarkdown>
                     </div>
                   )}
                </div>
              </>
